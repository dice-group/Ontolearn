...
class FTDL(TDL):
    ...
    def construct_owl_expression_from_tree(
        self, c: any, X: pd.DataFrame, y: pd.DataFrame
        ...
        for sequence_of_reasoning_steps, pos in zip(
            make_iterable_verbose(
                explain_inference(c, X=vector_representation_of_positive_examples),
    ...
    def fit(self, learning_problem: PosNegLPStandard = None, max_runtime: int = None):
        ...
        for c in self.clf.estimators_:
            self.conjunctive_concepts.append(
                self.construct_owl_expression_from_tree(c, X, y)
            )
        ...
        for tree_conjunctive_concepts in self.conjunctive_concepts:
            ...
            self.disjunction_of_conjunctive_concepts = concepts_reducer(
                concepts=tree_conjunctive_concepts, reduced_cls=OWLObjectUnionOf
            )
            self.tree_disjunctive_concepts.append(
                self.disjunction_of_conjunctive_concepts
            )
    ...
    def best_hypotheses(
        ...
        if n == 1:
            ...
            scores = []
            if reduce:
                return concepts_reducer(
                    self.tree_disjunctive_concepts, reduced_cls=OWLObjectUnionOf
                )
            for tdc in self.tree_disjunctive_concepts:
