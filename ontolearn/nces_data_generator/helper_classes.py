...
class RDFTriples:
    ...
    def export_triples(self, export_folder_name='triples'):
        ...
        if os.path.isfile(os.path.join(self.source_kg_path[:self.source_kg_path.rfind("/")], export_folder_name,
                                       "train.txt")):
            print("\n*** Embedding triples exist ***\n")
            return
        train_file = open("%s/train.txt" % os.path.join(self.source_kg_path[:self.source_kg_path.rfind("/")],
                                                        export_folder_name), mode="w")
...
class KB2Data:
    ...
    non-redundant concepts. Finally, we aim at training a deep neural network to predict the syntax of concepts from
    their instances. Hence, we export each concept and its instances (eventually positive and negative examples) into
    json files.
    """

    def __init__(self, path, rho_name="ExpressRefinement", depth=5, max_child_length=25, refinement_expressivity=0.6,
                 downsample_refinements=True, k=10, num_rand_samples=150, min_num_pos_examples=1):
        self.path = path
        ...
        rho = ExpressRefinement(knowledge_base=self.kb, max_child_length=max_child_length, sample_fillers_count=k,
