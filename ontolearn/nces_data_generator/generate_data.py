...
parser.add_argument('--num_rand_samples', type=int, default=200,
                    help='The number of random samples at each step of the generation process')
...
parser.add_argument('--rho', type=str, default='ExpressRefinement', choices=['ExpressRefinement'],
                    help='Refinement operator to use')
...
for kb in args.kbs:
    ...
    DataGen = KB2Data(path=f'{base_path}NCESData/{kb}/{kb}.owl', rho_name=args.rho, depth=args.depth,
                      max_child_length=args.max_child_len, refinement_expressivity=args.refinement_expressivity,
                      downsample_refinements=True, num_rand_samples=args.num_rand_samples, min_num_pos_examples=1)
