...
def execute(args):
    ...
    if args.ratio_sample_object_prob:
        object_properties = {i for i in random.sample(population=list(object_properties),
                                                      k=max(1, int(len(
                                                          object_properties) * args.ratio_sample_nc)))}
    ...
    if args.ratio_sample_nc:
        ...
        nc = {i for i in
              random.sample(population=list(nc), k=max(1, int(len(nc) * args.ratio_sample_nc)))}
    ...
    df = pd.read_csv(args.path_report, index_col=0,
                     converters={'Symbolic_Retrieval': lambda x: ast.literal_eval(x),
