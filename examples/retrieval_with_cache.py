...
#5, 16, 32, 128, 256, 512, 700, 800, 1024, , "KGs/Family/family.owl" .1, .2, .4, .8, 
parser = argparse.ArgumentParser()
parser.add_argument('--cache_size_ratios', type=list, default=[.1, .2, .4, .8], help="cache size is proportional to num_concepts, cache size = k * num_concepts")
...
def get_cache_size(list_k):
    
    data_size = len(concept_generator(args.path_kg[0]))
...
for path_kg in args.path_kg:
    for cache_size in get_cache_size(args.cache_size_ratios):
