...
# CONFIG
# ----------------------
SPARQL_ENDPOINT = "http://localhost:3030/imdb_100/sparql"
...
# Add a fetch Personas list. You should start the server via unicorn.
# 1. Clone: git clone https://github.com/Louis-Mozart/Datasets/.git && cd Datasets
# 2. conda activate venv_personas (If the virtual environemnt does not exists, do conda create -n venv_personas --no-default-packages && conda activate venv_personas && pip install -r requirements.txt )
# 3. uvicorn server:app --reload
# Then you can fetch personas via http requests as below 

def fetch_persona_list(limit: int = 10, host: str = "http://localhost:8000") -> list:
    response = requests.get(f"{host}/personas_list?limit={limit}")
    response.raise_for_status()
    namespace = {}
    exec(response.text, namespace)
    return namespace["persona_list"]
...
if __name__ == "__main__":
    ...
    persona_list = fetch_persona_list(limit=10, host="http://localhost:8000")
    # print(persona_list)
    # exit(0)
