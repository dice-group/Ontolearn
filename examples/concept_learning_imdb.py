...
def dl_concept_learning(args):
    ...
    # exit(0)
    examples = next((item for item in settings["problems"] if item["threshold"] == 2.0), None)

    # print(examples)
    # exit(0)

    # for examples in settings['problems']:
    if examples is not None:
        # threshold = examples["threshold"]
        p = set(examples['positives'])
        n = set(examples['negatives'])
        print('\n\n')

        str_target_concept = f"rating >= {2.0}"
        ...
        typed_pos = set(map(lambda uri: OWLNamedIndividual(IRI.create(uri)), p))
        typed_neg = set(map(lambda uri: OWLNamedIndividual(IRI.create(uri)), n))
